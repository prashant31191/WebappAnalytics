...
$.fn.thumbnail = function(options, Dispatcher){
    var wrapper = this,
        ...
        events = 'webkitTransitionEnd drag swipe dragstart dragend tap mousewheel transform release ' + $.TOUCHSTART + ' ' + $.TOUCHEND;
    ...
    var itemHelper = {
        ...
        setContent: function(absIndex, obj){
            if(this.has(absIndex) && obj){
                this.get(absIndex).removeClass('unloaded')[0].childNodes[0].src = obj.url_s;
            }
        },
        ...
        add: function(absIndex){
            ...
            if(!options.data[absIndex] || !options.data[absIndex].url_sq){
                img = '<img src="/assets/js/component/gallery/images/background.png" data-index="'+absIndex+'" height="'+options.imageHeight+'" width="'+options.imageWidth+'" draggable="false" />';
                list = $('<li data-index="'+absIndex+'" class="unloaded"></li>');
            }else{
                img = '<img src="'+options.data[absIndex].url_s+'" data-index="'+absIndex+'" height="'+options.imageHeight+'" width="'+options.imageWidth+'" draggable="false" />'
    ...
    function moveTo(index, trigger){
        ...
        if(deltaX < 0){
            ...
            move(-(thumbnailWidth - containerWidth), moveOpt);
        }else{
            move(-deltaX + 5, moveOpt);
        }
    ...
    function thumbHandler(e){
        if(e.gesture){
            ...
            if(e.gesture.deltaX < 0 && thumbnailWidth <= containerWidth) return;
        }
        switch(e.type) {
            case 'dragstart':
